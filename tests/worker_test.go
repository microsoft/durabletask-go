...
func Test_TryProcessSingleOrchestrationWorkItem_BasicFlow(t *testing.T) {
	...
	wi := &backend.OrchestrationWorkItem{
		InstanceID: "test123",
		NewEvents:  []*protos.HistoryEvent{helpers.NewExecutionStartedEvent(-1, "MyOrch", "test123", nil, nil)}}
...
func Test_TryProcessSingleOrchestrationWorkItem_ExecutionStartedAndCompleted(t *testing.T) {
	...
	startEvent := helpers.NewExecutionStartedEvent(-1, "MyOrchestration", string(iid), nil, nil)
	wi := &backend.OrchestrationWorkItem{
		InstanceID: iid,
